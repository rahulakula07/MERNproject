...
res.status(201).json({
    ...
    onsole.log("error in signup controller")
...
export const logout=async (req,res)=>{
   ...
   res.json({message:"logged out sucessfully"})
    ...
    res.status(500).json({message:"server error",erroe:error.messaage})
   }
}
...
const accessToken=jwt.sign({userId:decode.userId},process.env.ACCESS_TOKEN_SECRET,{expiresIn:"15m"})
    ...
    res.json({message:"Tokenrefreshed sucessfully"})
    }
    catch(error) {
        console.log("error in refreshToken controller");
    res.status(500).json({message:"server error",error:error.messaage})
}
...
try {
    ...
    res.status(500).json({messaage:"server error",error:error.messaage})
}
}